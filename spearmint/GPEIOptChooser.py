...
def optimize_pt(c, b, comp, pend, vals, model):
    ret = spo.fmin_l_bfgs_b(model.grad_optimize_ei_over_hypers, c.flatten(), args=(comp, pend, vals), bounds=b, disp=0)
    return ret[0]
...
class GPEIOptChooser:
    ...
    def dump_hypers(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False) as fh:
            cPickle.dump({'dims': self.D, 'ls': self.ls, 'amp2': self.amp2, 'noise': self.noise, 'mean': self.mean }, fh)
    ...
    def _real_init(self, dims, values):
        ...
        if os.path.exists(self.state_pkl):
            with open(self.state_pkl, 'r') as fh:
    ...
    def next(self, grid, values, durations,
        ...
        if self.mcmc_iters > 0:
            ...
            results = [pool.apply_async(optimize_pt, args=(c, b, comp, pend, vals, deepcopy(self))) for c in cand2]
            for res in results:
            ...
            for i in xrange(0, cand2.shape[0]):
                ret = spo.fmin_l_bfgs_b(self.grad_optimize_ei, cand2[i,:].flatten(), args=(comp, vals, True), bounds=b, disp=0)
                cand2[i,:] = ret[0]
            cand = np.vstack((cand, cand2))
